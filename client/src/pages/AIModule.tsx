...
import {
  Brain, Upload, Loader2, CheckCircle, XCircle, Pencil,
  AlertTriangle, Image as ImageIcon, History, Sparkles,
} from "lucide-react";
...
export default function AIModule() {
  ...
  const analyzeMutation = useMutation({
    mutationFn: async () => {
      ...
      const { path, signedUrl } = await uploadAndSign("ultrasound-images", fileName, selectedFile);
      ...
      // Call edge function
      const { data, error } = await supabase.functions.invoke("analyze-ultrasound", {
    ...
    onSuccess: ({ analysis: result, imageUrl }) => {
      ...
      toast({ title: "Análisis completado", description: `Patrón: ${result.pattern}` });
    },
    onError: (err: Error) => {
  ...
  const updateStatus = async (id: string, status: string, finalData?: { findings?: unknown; interpretation?: string }) => {
    const updateObj: Record<string, unknown> = { status };
    if (finalData?.findings) updateObj.final_findings = JSON.parse(JSON.stringify(finalData.findings));
    if (finalData?.interpretation) updateObj.final_interpretation = finalData.interpretation;

    await supabase.from("ai_predictions").update(updateObj).eq("id", id);
    queryClient.invalidateQueries({ queryKey: ["ai-predictions"] });
    toast({ title: `Predicción ${status === "accepted" ? "aceptada" : status === "rejected" ? "rechazada" : "editada"}` });
  };
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            <h1 className="font-display text-3xl font-bold">IA — Interpretación Asistida</h1>
            <p className="text-sm text-muted-foreground">
              Análisis de imágenes ecográficas pulmonares con inteligencia artificial
            </p>
          </div>
        </div>
        <div className="mt-3 p-3 rounded-lg bg-amber-500/10 border border-amber-200">
          <p className="text-xs text-amber-800 flex items-center gap-2">
            ...
            <span>
              <strong>Herramienta de apoyo clínico.</strong> Los resultados de la IA son orientativos y no reemplazan
              el juicio del profesional de salud. Toda interpretación debe ser validada clínicamente.
            </span>
          </p>
        </div>
      </div>
            ...
            <CardContent className="p-5">
              ...
              <div className="space-y-3">
                ...
                <div
                  ...
                  onClick={() => document.getElementById("lus-image-input")?.click()}
                >
                  {previewUrl ? (
                    <img src={previewUrl} alt="Ecografía" className="max-h-64 mx-auto rounded" />
                  ) : (
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground/60">JPG, PNG — máx 10MB</p>
                    </div>
                  )}
                  <input
                    id="lus-image-input"
                  ...
                  <CardContent className="p-5 space-y-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-sm font-semibold flex items-center gap-2">
                        <Brain className="h-4 w-4 text-primary" /> Resultado del análisis
                      </h3>
                      <div className="flex gap-2">
                        <Badge variant="outline" className={confidenceColor[analysis.confidence] || ""}>
                          {analysis.confidence}
                        </Badge>
                        <Badge variant="secondary">Score: {analysis.zone_score}/3</Badge>
                      </div>
                    </div>

                    {/* Pattern */}
                    <div className="p-3 rounded-lg bg-primary/5">
                      <p className="text-xs text-muted-foreground mb-1">Patrón sugerido</p>
                      <p className="font-semibold">{analysis.pattern}</p>
                    </div>

                    {/* Findings */}
                    <div>
                      <p className="text-xs text-muted-foreground mb-2">Hallazgos detectados</p>
                      <div className="flex flex-wrap gap-1.5">
                        ...
                        {Object.values(analysis.findings).every((v) => !v) && (
                          <p className="text-xs text-muted-foreground">Sin hallazgos positivos</p>
                        )}
                      </div>
                    </div>
                    ...
                    {/* Interpretation */}
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">Interpretación</p>
                      {isEditing ? (
                        <Textarea
                    ...
                    <div className="flex gap-2 pt-2 border-t">
                      <Button
                        ...
                        variant="default"
                        onClick={() =>
                          toast({ title: "Sugerencia aceptada", description: "Hallazgos guardados." })
                        }
                      >
                      ...
                      >
                        <Pencil className="h-3.5 w-3.5 mr-1" /> {isEditing ? "Guardar edición" : "Editar"}
                      </Button>
                      <Button
                        size="sm"
                        variant="ghost"
            ...
            <CardContent className="p-5">
              ...
              {history.length === 0 ? (
                ...
                <div className="space-y-3 max-h-[600px] overflow-auto">
                  {history.map((pred) => (
                    <div key={pred.id} className="p-3 rounded-lg border text-sm space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex gap-2 items-center">
                          ...
                          {pred.confidence_level && (
                            <Badge variant="outline" className={`text-[10px] ${confidenceColor[pred.confidence_level] || ""}`}>
                              {pred.confidence_level}
                            </Badge>
                          )}
                        </div>
                        <span className="text-[10px] text-muted-foreground">
                          {new Date(pred.created_at).toLocaleDateString("es-CL")}
                        </span>
                      </div>
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-xs">{pred.suggested_pattern}</p>
                          <p className="text-xs text-muted-foreground line-clamp-2">
                            {pred.final_interpretation || pred.suggested_interpretation}
                          </p>
                          {pred.operator && (
                      ...
                      {pred.status === "pending" && (
                        <div className="flex gap-1.5">
                          <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => updateStatus(pred.id, "accepted")}>
                            <CheckCircle className="h-3 w-3 mr-1" /> Aceptar
                          </Button>
                          <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => updateStatus(pred.id, "rejected")}>
