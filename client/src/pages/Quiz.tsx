...
export default function Quiz() {
 ...
 const pulmoContext = useMemo(() => {
  if (finished)
   return `Página: Quiz Interactivo. Quiz finalizado. Puntuación: ${score}/${quizQuestions.length}.`;
  let ctx = `Página: Quiz Interactivo. Pregunta ${currentQ + 1}/${quizQuestions.length}. Modalidad: ${q.modality}. Pregunta: "${q.question}". Opciones: ${q.options.map((o, i) => `${i + 1}) ${o}`).join(", ")}.`;
  if (showAnswer) {
   const wasCorrect = selected === q.correctIndex;
   ctx += ` El estudiante respondió: "${q.options[selected!]}" — ${wasCorrect ? "CORRECTO ✅" : "INCORRECTO ❌ (la respuesta correcta era: " + q.options[q.correctIndex] + ")"}. Explicación: ${q.explanation}`;
  }
  return ctx;
 }, [currentQ, q, showAnswer, selected, finished, score]);
