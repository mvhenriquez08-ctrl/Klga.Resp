...
import { nivModes, nivParameters } from "@/data/nivSection";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Badge } from "@/components/ui/badge";
import { Wind, BookOpen, Settings } from "lucide-react";
...
export default function NIVLibrary() {
 const [selectedMode, setSelectedMode] = useState(nivModes[0].id);
 const mode = nivModes.find(m => m.id === selectedMode)!;
     ...
     <div className="flex items-center gap-5">
      ...
      <div>
       <h1 className="text-2xl font-display font-bold text-primary-foreground tracking-tighter uppercase leading-none">
        Biblioteca <span className="text-blue-400">VMNI</span>
       </h1>
       <p className="text-primary-foreground/60 font-sans font-bold text-xs uppercase tracking-[0.3em] mt-2">
        Ventilación Mecánica No Invasiva y Soporte Avanzado
       </p>
      </div>
     </div>
    </div>
    ...
    <Tabs defaultValue="modes">
     <TabsList className="mb-6">
      <TabsTrigger value="modes" className="gap-2">
       <BookOpen className="h-4 w-4" /> Modos VMNI
      </TabsTrigger>
