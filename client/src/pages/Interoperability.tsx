...
export default function Interoperability() {
  ...
  // Search form
  const [searchForm, setSearchForm] = useState({ rut: "", name: "", external_id: "" });
  ...
  const saveConnection = async () => {
    ...
    if (configForm.auth_method === "bearer_token") metadata.token = configForm.token;
    ...
    const { error } = await supabase.from("institutional_connections").insert(payload as any);
    if (error) {
      toast.error("Error al guardar conexión");
    } else {
      toast.success("Conexión institucional guardada");
      logAudit("create_connection", "institutional_connection", undefined, { institution: configForm.institution_name });
      setConfigForm({
        institution_name: "", system_type: "fhir_r4", base_url: "", auth_method: "bearer_token",
        token: "", username: "", password: "", api_key_header: "", api_key_value: "",
      });
      loadConnections();
    }
    setLoading(false);
  ...
  const processPendingSync = async () => {
    ...
    for (const item of pending) {
      try {
        ...
        if (error) throw error;
        await supabase.from("pending_sync_queue").update({ status: "completado" } as any).eq("id", item.id);
        ok++;
      } catch {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-display font-bold text-foreground">Interoperabilidad</h1>
          <p className="text-sm text-muted-foreground">
            Conexión con sistemas institucionales · HL7 FHIR R4 · API REST · CSV/JSON
          </p>
        </div>
      ...
      <Tabs defaultValue="config">
        ...
        <TabsContent value="config" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setConfigForm({ ...configForm, institution_name: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setConfigForm({ ...configForm, system_type: v })}
                  >
                  ...
                  <>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) => setConfigForm({ ...configForm, auth_method: v })}
                      >
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                          {AUTH_METHODS.map((m) => (
                      ...
                      <>
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setConfigForm({ ...configForm, api_key_header: e.target.value })
                            }
                          />
                        </div>
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setConfigForm({ ...configForm, api_key_value: e.target.value })
                            }
                          />
                        </div>
                      </>
              ...
              <Button onClick={saveConnection} disabled={loading || !configForm.institution_name}>
                  ...
                  <TableBody>
                    {connections.map((conn) => (
                      <TableRow key={conn.id}>
                        <TableCell className="font-medium">{conn.institution_name}</TableCell>
                        <TableCell>
                          {SYSTEM_TYPES.find((t) => t.value === conn.system_type)?.label || conn.system_type}
                        </TableCell>
                        <TableCell><StatusBadge status={conn.connection_status} /></TableCell>
                        <TableCell className="text-sm text-muted-foreground">
                          {conn.last_sync_at
                            ? new Date(conn.last_sync_at).toLocaleString("es-CL")
                            : "Nunca"}
                        </TableCell>
              ...
              {selectedConnection && (
                <p className="text-xs text-muted-foreground mt-2">
                  ...
                  <span className="font-medium text-foreground">
                    {connections.find((c) => c.id === selectedConnection)?.institution_name}
                  </span>
                </p>
              )}
            </CardContent>
        ...
        <TabsContent value="search" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Search className="h-5 w-5" /> Buscar Pacientes en Sistema Externo
              </CardTitle>
              <CardDescription>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setSearchForm({ ...searchForm, rut: e.target.value })}
                      />
                    </div>
                    <div className="space-y-2">
                      <Label>Nombre</Label>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setSearchForm({ ...searchForm, external_id: e.target.value })
                        }
                      />
                    </div>
                  </div>
                  <Button onClick={searchPatients} disabled={loading}>
                    <Search className="h-4 w-4 mr-1" /> Buscar en Sistema Externo
                  </Button>
                </>
              )}
                  ...
                  <TableBody>
                    {searchResults.map((p, idx) => (
                      <TableRow key={idx}>
                        ...
                        <TableCell className="text-xs">{p.external_id || "—"}</TableCell>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground">
                  CSV: columnas rut, nombre, fecha_nacimiento, sexo, servicio, cama, fecha_ingreso, diagnostico
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {!selectedConnection ? (
                  ...
                  <>
                    <p className="text-sm text-muted-foreground">
                      Los informes se envían desde la ficha del paciente con el botón
                      "Enviar a ficha clínica".
                    </p>
                    <Button variant="outline" onClick={() => queueForSync("export_report", {})}>
                      <Clock className="h-4 w-4 mr-1" /> Encolar para envío posterior
                    </Button>
                  </>
                )}
              </CardContent>
        ...
        <TabsContent value="history">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Registro de auditoría de todas las operaciones de interoperabilidad
              </CardDescription>
            </CardHeader>
            <CardContent>
              {syncLogs.length === 0 ? (
