...
import { Upload, Camera, ZoomIn, Sun, Contrast, RotateCw, Loader2, AlertTriangle, CheckCircle } from "lucide-react";
...
export default function VMCurvesAssistant() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("analyze-curves", {
        body: {
          image_base64: imageBase64,
          ventilator_params: Object.keys(params).length > 0 ? params : undefined,
        },
      });
  ...
  const detectedFindings = analysis ? Object.entries(analysis.findings).filter(([, v]) => v) : [];
        ...
        <div className="mx-auto max-w-6xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-sm text-primary-foreground/60">
              Sube una foto de las curvas del ventilador y obtén interpretación asistida por IA.
            </p>
          </motion.div>
        </div>
      </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-display">Imagen de Curvas</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              {imageUrl && (
                <>
                  {[
                    { label: "Zoom", value: zoom, set: setZoom, min: 25, max: 400, unit: "%" },
                    { label: "Brillo", value: brightness, set: setBrightness, min: 0, max: 200, unit: "%" },
                    { label: "Contraste", value: contrast, set: setContrast, min: 0, max: 200, unit: "%" },
                  ].map((c) => (
                    <div key={c.label}>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="font-medium">{c.label}</span>
                        <Badge variant="outline" className="text-[10px]">{c.value}{c.unit}</Badge>
                      </div>
                      <Slider value={[c.value]} onValueChange={([v]) => c.set(v)} min={c.min} max={c.max} />
                    </div>
                  ))}
                  <Button variant="outline" size="sm" className="w-full gap-1 text-xs" onClick={() => setRotation((r) => (r + 90) % 360)}>
              ...
              <div className="border-t pt-3 space-y-3">
                <p className="text-xs font-semibold">Parámetros del paciente (opcional)</p>
                <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <div key={f.label}>
                      ...
                      <Input value={f.value} onChange={(e) => f.set(e.target.value)} className="h-7 text-xs mt-0.5" />
                    </div>
                  ))}
                </div>
              </div>

              <Button onClick={handleAnalyze} disabled={!imageBase64 || analyzing} className="w-full gap-2">
                {analyzing ? <><Loader2 className="h-4 w-4 animate-spin" /> Analizando...</> : "Analizar con IA"}
              </Button>
            </CardContent>
          </Card>
          ...
          <Card className="lg:col-span-1">
            <CardContent className="p-0">
              {imageUrl ? (
                ...
                <div className="flex flex-col items-center justify-center min-h-[400px] bg-muted/30 cursor-pointer" onClick={() => fileRef.current?.click()}>
                  <Camera className="h-16 w-16 text-muted-foreground/20 mb-4" />
                  <p className="text-sm text-muted-foreground">Toma una foto o sube una imagen</p>
                  <p className="text-xs text-muted-foreground/60 mt-1">de las curvas del ventilador</p>
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <div className="space-y-4">
            {analysis ? (
              <>
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-display flex items-center gap-2">
                      ...
                      <Badge variant={analysis.confidence === "alta" ? "default" : "secondary"} className="text-[10px]">
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex flex-wrap gap-1">
                      ...
                      <Badge className="text-[10px] bg-primary/10 text-primary">{analysis.ventilation_mode}</Badge>
                    </div>
                    <div>
                      <h4 className="text-sm font-semibold mb-1">Hallazgo Principal</h4>
                      <p className="text-sm text-muted-foreground">{analysis.main_finding}</p>
                    </div>
                    <div>
                      <h4 className="text-sm font-semibold mb-1">Explicación</h4>
                ...
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm font-display">Posibles Causas</CardTitle>
                  </CardHeader>
                  <CardContent>
                ...
                <div className="rounded-lg bg-warning/5 border border-warning/20 p-3">
                  <p className="text-[10px] text-muted-foreground">
                    <strong className="text-warning">⚠️</strong> Interpretación orientativa. No reemplaza el juicio clínico del profesional.
                  </p>
                </div>
              </>
            ) : (
              <Card>
                <CardContent className="flex flex-col items-center justify-center min-h-[200px]">
                  ...
                  <p className="text-sm text-muted-foreground text-center">
                    Sube una imagen de las curvas del ventilador y presiona "Analizar con IA"
                  </p>
                </CardContent>
              </Card>
            )}
