...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  const sanitizeCssIdentifier = (identifier: string) => {
    return identifier.replace(/[^a-zA-Z0-9-]/g, "");
  };

  const sanitizeColor = (color: string) => {
    // Basic sanitization, allows hex, rgb, rgba, hsl, hsla, and color names.
    // Disallows characters that could break out of the CSS property value.
    if (/[;{}]/.test(color)) {
      return "";
    }
    return color;
