...
function generatePVLoop(vt: number, peep: number, compliance: number) {
  const data = [];
  const steps = 50;
  for (let i = 0; i <= steps; i++) {
    const vol = (vt * i) / steps;
    const pInsp = peep + vol / compliance + 2 * Math.sin((Math.PI * i) / steps);
    data.push({ pressure: +pInsp.toFixed(1), volume: +vol.toFixed(0) });
  }
  for (let i = steps; i >= 0; i--) {
    const vol = (vt * i) / steps;
    const pExp =
      peep + vol / compliance - 1.5 * Math.sin((Math.PI * i) / steps);
    data.push({ pressure: +pExp.toFixed(1), volume: +vol.toFixed(0) });
  }
  return data;
}
...
export default function VMCurves() {
  ...
  const ie = eRatio / iRatio; // effective E ratio when I=1
  const peakFlow = 60;
  ...
  const pvLoop = useMemo(
    () => generatePVLoop(vt, peep, compliance),
    [vt, peep, compliance]
  );
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <h1 className="font-display text-2xl font-bold text-primary-foreground">
                Curvas y Loops Ventilatorios
              </h1>
            </div>
            <p className="text-sm text-primary-foreground/60">
              Visualización interactiva de curvas de presión, flujo, volumen y
              loops P-V.
            </p>
          </motion.div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-4">
          ...
          <Card className="lg:col-span-1">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-display">Parámetros</CardTitle>
            </CardHeader>
