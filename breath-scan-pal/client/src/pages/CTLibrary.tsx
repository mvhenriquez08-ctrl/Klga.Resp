...
export default function CTLibrary() {
  ...
  const filtered =
    filter === "all"
      ? ctPatterns
      : ctPatterns.filter(p => p.category === filter);
        ...
        <div className="mx-auto max-w-6xl">
          ...
          >
            ...
            <p className="text-sm text-primary-foreground/60">
              Biblioteca de patrones TC, interpretación sistemática y casos
              clínicos.
            </p>
          </motion.div>
        </div>
        ...
        <Tabs defaultValue="patterns">
          <TabsList>
            <TabsTrigger value="patterns" className="gap-1">
              <BookOpen className="h-3 w-3" /> Patrones TC
            </TabsTrigger>
            <TabsTrigger value="interpretation" className="gap-1">
              <CheckSquare className="h-3 w-3" /> Interpretación
            </TabsTrigger>
          </TabsList>
            ...
            <Card className="mb-6">
              <CardHeader className="py-3">
                <CardTitle className="text-sm font-display flex items-center gap-2">
                  <Search className="h-4 w-4" /> Lectura Sistemática de TC de
                  Tórax
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-xs text-muted-foreground mb-4">
                  La interpretación de la TC de tórax debe ser sistemática,
                  evaluando todas las estructuras en diferentes ventanas de
                  atenuación. Se recomienda seguir siempre el mismo orden para
                  no omitir hallazgos.
                </p>
              </CardContent>
            </Card>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          {selected && (
            <>
              ...
              <Accordion
                type="multiple"
                defaultValue={["desc", "findings", "ddx"]}
                className="space-y-1"
              >
                <AccordionItem value="desc">
                  ...
                  <AccordionContent>
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      <strong>Mecanismo:</strong> {selected.mechanism}
                    </p>
                  </AccordionContent>
                </AccordionItem>
