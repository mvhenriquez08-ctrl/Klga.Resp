...
async function processAllPDFs() {
    ...
    for (const filePath of allFiles) {
        ...
        // --- NUEVO: Verificar si el archivo ya fue subido para no repetir y ahorrar cuota ---
        const { data: existing } = await supabase
            .from('document_chunks')
            .select('id')
            .eq('document_name', file)
            .limit(1);

        if (existing && existing.length > 0) {
            console.log(`\n⏩ Saltando: ${file} (Ya está guardado en la base de datos)`);
            continue;
        }
        ...
        for (let i = 0; i < chunks.length; i += batchSize) {
            const chunkBatch = chunks.slice(i, i + Math.min(batchSize, chunks.length - i));
            let retryCount = 0;
            const maxRetries = 5;
            let success = false;
            ...
            while (retryCount < maxRetries && !success) {
                try {
                    ...
                    await new Promise(r => setTimeout(r, 15000)); // Pausa de 15s
                } catch (err) {
        ...
        console.log(`\n✅ ${file} procesado exitosamente.`);
    }
    console.log("\n🚀 Pipeline finalizado. Tu base de datos está lista.");
}
